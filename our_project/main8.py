...
if __name__=="__main__":
    ...
    for index in range(len(windows_final)):
        ...
        boxes_dicts.append(dict(image_box=image_box, center=pixels_center))
        
    ### 3. CLASSIFICATION
    ## 3.1 DIGIT CLASSIFIER: TRANSLATED/SCALED/ROTATED MNIST CNN [see the function 'classify_number']
    ...
    char_imgs = [dct['image_box'] for dct in boxes_dicts]
    char_counter = 0
    closest_index_previous = None
    ...
    for i in range(len(frames)):
        ...
        if ((closest_distance < 50) and (closest_index != closest_index_previous)):  # Should we take a larger/smaller threshold?
        
            closest_index_previous = closest_index
            closest_img = char_imgs[closest_index]
        
            if char_counter % 2 == 0: # The next character is a digit
                closest_char = classify_number(closest_img)
            else: # The next character is an operator
                closest_char = classify_operator(closest_img) 
